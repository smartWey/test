...
在工具或 IDE 中，“Skills”通常指可调用的能力集合（如文件读写、搜索、运行命令等）。它描述了系统能“做什么”，并决定自动化或辅助功能的边界。

### MCP（Model Context Protocol）

MCP 是一种让模型与外部工具/服务安全、标准化交互的协议。它定义了上下文、能力与调用方式，便于不同工具在同一工作流中协同。

### 使用场景（简要）

1. **统一接入**：通过 MCP 把多个工具暴露为可调用能力。
2. **受控执行**：基于 Skills 列表决定可执行的操作范围。
3. **自动化流程**：在编辑器中串联“搜索 → 修改 → 验证”的任务链。
